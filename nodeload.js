...
var LOG = require('nlogger').logger('nodeload');
}
...
// Re-define the latency/runtime collector to pull the internally measured time.
StatsCollectors['11main-runtime'] = function (params) {
    var self = this;
    self.stats = new Histogram(params);
    self.start = function(context) { context.start = new Date(); };
    self.end = function(context, http) {
        var elapsed = http.elapsed || (new Date() - context.start);
        self.stats.put(elapsed);
    };
};

// Define the thinktime collector.
StatsCollectors['thinktime'] = function (params) {
    var self = this;
    self.stats = new Histogram(params);
    self.start = function (ctx) { ctx.start = new Date(); };
    self.end = function (ctx, http) {
        var elapsed = http.thinktime || (new Date() - ctx.start);
        self.stats.put(elapsed);
    };
};
...
HttpServer.prototype.stop = function() {
    if (!this.running) { return; }
    this.running = false;
    this.connections.forEach(function(c) { if (c.destroy) { c.destroy(); } });
    this.server.close();
    this.server = null;
    this.emit('end');
};
...
//            console.log("numClients: " + spec.numClients)
//            console.log("numRequests: " + spec.numRequests)
//            console.log("timeLimit: " + spec.timeLimit)
//            console.log("targetRps: " + spec.targetRps)
//            console.log("delay: " + spec.delay)
//            console.log("")
//            console.log("-------------")
//            console.log("")
...
Slave.prototype.end = function() {
    var self = this, 
	    ...
	    if (self.client.destroy) { self.client.destroy(); }
...
SlaveNode.prototype.end = function() {
    ...
    if (this.masterClient_ && this.masterClient_.destroy) {
